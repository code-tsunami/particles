...
const char* WINDOWTITLE = { "OpenCL/OpenGL Particle System -- Sonam Kindy" };
...
// change from const int to macro so can change arg on compile if need be
#ifndef NUM_PARTICLES
#define NUM_PARTICLES 1024 * 1024 // up to 16 * 1024 * 1024 at least
#endif

// designate if testing - if 1 (true), quit when frame == 1000
#ifndef TESTING
#define TESTING 0
fprintf(stderr, "TESTING: %d", TESTING);
#endif


const int LOCAL_SIZE        = 32;
const char* CL_FILE_NAME    = { "kindys.cl" };
...
//
// structs we will need later to fill vertex buffers:
//
...
void Display( )
{
    ...
    // if testing show performance and start particle simulation (animation)
    #if TESTING == 1
    Buttons(GO);
    ...
    if( ShowPerformance )
    {
        ...
        float performance = (float)NUM_PARTICLES/ElapsedTime/1000000000.;
        if(performance > PeakPerformance && !isinf(performance))
            PeakPerformance = performance;

        sprintf( str, "Frame %8d -- %6.1f GigaParticles/Sec", Frame,  performance);
        ...
        // if testing quit automatically at 1000 frames
        #if TESTING == 1
        if(Frame == 1000) Quit();
        #endif 
    }
...
void InitCL( )
{
    ...
    cl_uint deviceCount; // num of devices (CPU + GPU) available on machine
    cl_device_id* devices; // array of devices

    // get first platform
    status = clGetPlatformIDs(1, &Platform, NULL);
    PrintCLError(status, "A valid Platform could not be found on this machine");

    // get device count
    status = clGetDeviceIDs(Platform, CL_DEVICE_TYPE_ALL, 0, NULL, &deviceCount);
    PrintCLError(status, "Could not determine the number of devices available on this Platform");

    // get all devices
    devices = new cl_device_id[deviceCount];
    status = clGetDeviceIDs(Platform, CL_DEVICE_TYPE_ALL, deviceCount, devices, NULL);
    PrintCLError(status, "Could not access the devices");

    // create a context for all devices
    cl_context Context = clCreateContext(NULL, deviceCount, devices, NULL, NULL, &status);
    PrintCLError(status, "Could not create a context on this platform");

    if(deviceCount == 3)
        Device = devices[2]; // explicitly choose device #3 which is my second GPU 
    else {
        status = clGetDeviceIDs(Platform, CL_DEVICE_TYPE_GPU, 0, &Device, NULL);
        PrintCLError(status, "Could not determine the number of devices available on this Platform");
    }
    ...
    if( status != CL_SUCCESS )
        fprintf( stderr, "clCreateCommandQueue failed with error code %d\n", status );
    ...
    delete [ ] devices; // free malloc for devices
}
...
void ResetParticles( )
{
    ...
    for( int i = 0; i < NUM_PARTICLES; i++ )
    {
        colors[i].r = Ranf( .1f, 1. );
        colors[i].g = Ranf( .1f, 1. );
        colors[i].b = Ranf( .1f, 1. );
        colors[i].a = 1.;
    }
    glUnmapBuffer( GL_ARRAY_BUFFER );
...
void Quit( )
{
    ...
    // print peak performance of frames if display/show performance opt selected
    if(ShowPerformance)
        printf("%d,%.3f\n", NUM_PARTICLES, PeakPerformance); 
...
{
        char filename[128];
        sprintf( filename, "kindys.%03d.particles", Frame );
